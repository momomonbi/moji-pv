...
1. Overview and principles (incl. clean-room rules)
...
8. Testing plan and originality check
...
11. **Originality by construction** (§1.4): new architecture, new vocabulary, new catalog, enforced by CI.

### 1.4 Clean-room rules (restated, binding for everyone)

v2 is an original work written only from `next/SPEC.md`, this file, general web/graphics/typography knowledge, and our
own files `src/11s_ai.js`, `src/11t_ai_audio.js`, `dev/ai_test.js`, `dev/ai_audio_test.js`, `dev/browser.py` (plus the two
MIT vendor files, used unmodified). Nobody working on v2 opens the JIZURA files listed in SPEC §0 — `src/0*.js`,
`src/10_audio.js`, `src/11_export.js`, `src/11p_*.js`, `src/11q_sets.js`, `src/12_ui.js`, `app/`, `ae/`, `cep/`, the root
`build.py`, `docs/EXPRESSION_PACKS.md` and `docs/jizura/`, `index.html`, `en/index.html`, `dev/*.js` (except the two AI tests named
above) and `dev/snapshots/` — and nobody reasons about how JIZURA works, not even to avoid it: every structure here is
justified from the spec alone. Part, theme and mood names are v2's own. The only code that ever reads the forbidden files
is the originality tooling (§8.4), which prints only file names, token positions and overlapping identifier names — never
code. There is no importer for old-app project files in v2.0; old projects move over by pasting their lyrics. Any future
importer must be written from a black-box sample export, never from forbidden code. Every PR description ends with the
line `Clean-room: I did not open any file listed in SPEC §0.` Third-party code (mp4-muxer, the AI SDK) stays in `vendor/`,
unmodified, behind exactly one adapter each (`export/muxer.js`, `ai/providers.js`), with its MIT notice kept in
`next/THIRD_PARTY_NOTICES.md` and the in-app About page.
...
```
next/
  SPEC.md  DESIGN.md  THIRD_PARTY_NOTICES.md
  build.py                         # stdlib-only Python 3.10+: lint, layer check, topo sort, inline, CSP, pages, --lab, --check
  index.html  en/index.html        # BUILD OUTPUT (committed by CI only)
  src/
  ...
  tests/
    ...
    node/    *.test.js                                  # node --test next/tests/node
    browser/ *.py                                       # Playwright via ../../dev/browser.py
    www/     lab.html (build output, not shipped)
    originality_names.py  originality_allow.txt  build_test.py
```

Vendor files stay where they are: `vendor/mp4-muxer.min.js` (defines global `Mp4Muxer` with `Muxer`, `ArrayBufferTarget`,
`StreamTarget`, `FileSystemWritableFileStreamTarget`) and `vendor/ai-sdk.min.js` (defines `window.AnthropicSDK`).
...
| L2 | `planner/*` | L0, L1 (`engine/text/*` only for metric-free functions: `breaker`, `vert` classes) |
...
### 2.6 `next/build.py` (FROZEN behaviour)

```
python3 next/build.py            # writes next/index.html and next/en/index.html
python3 next/build.py --check    # lint + layers + topo only; exit 1 on any problem; writes nothing
python3 next/build.py --lab      # also writes next/tests/www/lab.html (MV.DEV=true; boots 'ui/lab' instead of 'ui/boot')
```

1. Collect `next/src/**/*.js` and `next/src/ui/style.css`. For each `.js` except `define.js`, parse the header with
...
5. CSP (one line, FROZEN): `default-src 'none'; script-src 'sha256-A' 'sha256-B' 'sha256-C'; style-src 'sha256-S'
   ...
   https://api.anthropic.com; img-src data: blob:; media-src blob:; worker-src 'none'; base-uri 'none'; form-action 'none'`.
...
| `titleCard` | boolean | `true` when a title is set and the first line starts ≥ 1.5 s |
...
- A candidate part is eligible for auto picks when it passes `filters[kind].only` (if set) and `deny` (if set), and the
  season gate: season `any` → all parts (seasonal parts weighted ×0.5); a season → non-seasonal parts plus that season's
  parts (×1.5); `none` → non-seasonal parts only. Themes follow the same gate.
...
- `a`/`b` = the cut's visible window: `a = t0 − timing.lead`, `b = next cut's t0 + timing.tail` (or `t1 + tail` before a
  gap). `repT` = representative "hero" time (entrance finished, text fully visible) used for seeking and thumbnails.
- `fp` (fingerprint) = hash of everything the scene build needs: slots, `els`, text, emph, duration `b − a`, role, palette,
  faces, design size; plus `(a mod beatPeriod)` only when a chosen part declares `needs: ['beats']`. The scene cache
  rebuilds only cuts whose `fp` changed.
...
// migrate
parseFile(text) → { doc, side }    // JSON.parse + migrate + normalize + validate; throws MigrateError('bad-json'|'not-a-project'|'newer'|'invalid')
...
3. Exactly `[ti:…]`, `[ar:…]`, `[al:…]`, `[by:…]`, `[offset:…]`, `[re:…]`, `[ve:…]` (case-insensitive tag) → `meta`
   (`ti` → title, `ar` → artist; others ignored).
...
3. In each gap between consecutive anchors, pair remaining old/new rows in order, maximizing the total character-bigram
   Dice similarity of their keys (DP over the gap), accepting a pair only if Dice ≥ 0.5. Paired rows keep their id and are
   `edited` when the plain text differs.
4. Moves: a still-unpaired new row whose key is unique among all new rows and equals the key of a still-unpaired old row
   that is unique among all old rows takes that id.
...
solveTimes(lines, ctx) → { times: [{ id, t0, t1, by: { start: 'pin'|'lrc'|'auto', end: 'pin'|'auto' } }], duration, warnings }
  ...
  //         titleCard: seconds reserved before the first auto line (2.0 when the `titleCard` slot is pinned true, else 0) }
  // An auto title card never moves lines: the cutter fits it into [0, first line start) afterwards (§4.16.5).
```
Rules (FROZEN):
...
encodeWav(channels, sampleRate, rate = 16000) → Uint8Array                                   // port of 11t encodeWav (same bytes)
// host
...
phrases(text, lang) → [[a, b]]        // phrase ranges (文節 approximation). ja: a break after the particles
                                       // は が を に で と へ も の や か から まで より ね よ な さ, but only when the next grapheme is
                                       // not hiragana or the chunk is ≥ 6 graphemes; after 、 and 。; at spaces; at script changes
                                       // (kana/han ↔ Latin). en/ko: at spaces. zh: every han, grouped in 2s when > 8 graphemes.
...
- **theme**: pin, else weights `mood.themes[key] ?? 0.4`, times the season gate (§3.8), times `filters.theme`; Gumbel with
  `hash32('theme', moodSeed)`.
...
w(d)   = q6( d.weight                                       (default 1)
           ...
           × moodBias(d.tags)                               geometric mean of mood.tagBias[tag] ?? 1 over d.tags (1 when no tags)
           × gate                                           d.gate ? look.amounts[d.gate] : 1   (0 → out of the pool)
           × (theme.prefer[K]?.[d.key] ?? 1)
           × season                                         seasonal part: ×0.5 when season 'any', ×1.5 when it matches
           × (c.feat.impact && d.traits.impact ? 6 : 1)
           × recency                                        ×0.03 if d.key == previous cut's value of this slot;
                                                            ×0.35 if it appears in the 3 cuts before that; ×0.5 if d.family == previous
                                                            cut's family (factors multiply; locked/pinned neighbours count as facts)
...
- `filter.count`: `min(2, floor(0.8·max(amounts.glitch, amounts.chroma) + r))`, plus 1 on impact cuts when
  `amounts.flash > 0` (the chooser then favours filters with `traits.impact`).
...
- Special cuts: `title` when the `titleCard` slot is true (auto: title set and first line ≥ 1.5 s; pinned true with less
...
`from: 'rule'`; likewise `replaces.arrive` → B's arrive `instantShow`.
...
- **Stagger**: `staggerOf(env, target, order, each, unit)` ranks units (`lead` = reading order, i.e. left→right/top→bottom
  ...
  `stream(slotSeed,'order')` permutation; `word`/`line` = by word/line index; `sweepX/Y` = by position; `radial` = by
...
buildGround(seg, plan, svc) → Scene       // ground + atmos of one segment; its local time = t − seg.t0
...
env = {
  ...
  hints: { focus: Box, free: Box[] } | null,                     // ornaments and lens only
...
Engine = {
  ...
  thumb(ref: { kind, key, params? }, surface, { t, text?, theme?, aspect? })  // canned sample cut; deterministic
  ...
  stats() → { frameMs, stageMs: { behave, draw, post }, spriteBytes, surfaces, scenes }
  dispose()
}
...
### 4.22 AI adapter: `ai/*` (WP7) — port of `src/11s_ai.js` and `src/11t_ai_audio.js`

Behaviour is ported, not the data model: same providers, key handling, prompts' intent, JSON schemas' closedness,
validation rules and error codes; everything that touched line indices now works on ids and v2 commands.

#### 4.22.1 Providers (`ai/providers.js`, straight port)

`PROVIDERS` (gemini default `gemini-3.8-flash`; claude `claude-opus-5`, `claude-sonnet-5`, `claude-haiku-4-5`; key URLs and
prices as in 11s), `DEFAULT_PROVIDER`, `costUSD(provider, model, usage)`, `AIError(code, message, extra)`,
...
statuses mapped exactly as in 11s. Claude: through the SDK (`window.AnthropicSDK` injected by `ui/boot` via
`providers.setSDK()`), `dangerouslyAllowBrowser`, structured output, adaptive thinking except Haiku, the Opus server-side
fallback beta, SDK errors mapped as in 11s, media refused (`no_audio`). Error codes: `no_key auth rate model server
...
Schemas use v2 names (all objects closed, every property required, no numeric/string limits — the portability rule of
11s): look answers carry `theme, mood, season, amounts{motion,glitch,chroma,ornament,density,texture,groundSwitch}
(−1 = keep), flash ('keep'|'on'|'off' or boolean), palette{accent,shiftA,shiftB}, avoid[], allow[] ("kind.key"),
lines[{ i, arrange, arrive, depart, dwell, impact, emphasis[] }]`. Validation rules are those of 11s: unknown keys and
...
#### 4.22.6 Privacy and consent (unchanged behaviour)
...
when ≤ 14 MB, otherwise through the Files API (resumable upload, poll until ACTIVE ≤ 60 s) — ported from 11t
(`encodeWav`, `pickRate`, `toBase64`, `audioPart`, `uploadFile`, `seconds`, `lrcTag`). Keys live in `sessionStorage` by
...
| `spinIn` | 回転 | Spin in | playful fast | Glyphs spin half a turn while growing from nothing. |
...
| `tiltUp` | 起き上がり | Tilt up | playful retro | Glyphs stand up from lying flat on the baseline. |
| `glitchIn` | 乱入 | Glitch in | digital hard fast | Glyphs jitter sideways with color echoes, then lock in place (gate `glitch`). |
...
| `beatPulse` | 拍動 | Beat pulse | fast bold | The block pulses in scale on every beat (needs beats). |
...
| `spinOut` | 回転抜け | Spin out | playful fast | Mirror of spinIn. |
...
| `dotMatrix` | 点の列 | Dot matrix | cut | digital retro | | Rows of small dots beside the text that light up in sequence. |
...
| `waveLine` | 波線 | Wave line | cut | playful organic | | A wavy underline that ripples. |
...
| `lightLeak` | 光漏れ | Light leak | light | soft retro | Warm light leaks drifting in from one edge. |
...
| `inkBleed` | 墨にじみ | Ink bleed | text | depart | organic literary | The new text bleeds in through a spreading ink-blot mask. |
| `pushSlide` | 押し出し | Push slide | world | depart | fast bold | The next world pushes the previous one out sideways. |
...
| `zoomThrough` | ズーム抜け | Zoom through | world | depart | fast bold | Zoom into the old cut and out of the new one. |
...
| `whipPan` | 振り | Whip pan | world | depart | fast | A fast motion-blurred pan into the next cut. |
...
| `cyanPrint` | 青写真 / Cyan print | #0E3A6B #164B85 #EAF2FA #9FD3FF #5FA8E8 #F2C14E #7FA0C4 | Shippori Antique · Space Grotesk (400, antique) | Hina Mincho · Instrument Serif (400, mincho) | BIZ UDPGothic · IBM Plex Sans (400, gothic) | paperTooth | |
...
| `silverReel` | 銀幕 / Reel | .45 / .05 / .15 / .30 / .35 / .70 / .30 / .20 / .15 / .80 / .35 | slow 1.5 dark 1.5 serious 1.4 minimal 1.2 · busy .5 playful .5 | emberGlow 1.6, nightTram 1.4, brassLamp 1.3, mossStone 1.2, mapleInk 1.2 | .50 / .30 | cinemaBars .9, grainFilm .8, edgeShade .8, lightLeak .3 | .8 |
| `printColumn` | 誌面 / Column | .40 / 0 / .05 / .55 / .45 / .45 / .35 / .10 / .05 / .25 / .45 | literary 1.7 minimal 1.6 serious 1.3 bold 1.3 · wet .5 busy .6 | monoPress 2, risoPink 1.4, sumiWashi 1.3, frostGlass 1.2 | .30 / .40 | dotScreen .5, grainFilm .4, duoTone .3 | .9 |
| `heartAche` | 胸の奥 / Ache | .45 / .10 / .25 / .45 / .35 / .55 / .35 / .25 / .10 / .50 / .40 | soft 1.6 wet 1.5 slow 1.4 literary 1.3 · hard .5 digital .4 | snowLantern 1.5, sakuraFog 1.4, mapleInk 1.3, nightTram 1.2, tidePool 1.1 | .45 / .40 | glowSpill .6, softVeil .5, lightLeak .4, grainFilm .4 | .9 |
...
| `dreamHaze` | 霞 / Haze | .35 / .05 / .20 / .50 / .30 / .45 / .30 / .15 / 0 / .40 / .35 | airy 1.8 soft 1.6 wet 1.4 organic 1.3 · hard .4 digital .4 | frostGlass 1.5, sakuraFog 1.5, tidePool 1.3, sodaFloat 1.1 | .50 / .30 | glowSpill .7, softVeil .6, lightLeak .5 | .9 |
...
| Element | Fields |
|---|---|
| 文字 Text | 書体 `text.face`, 大きさ `text.scale`, 文字色 `text.ink`, 強調色 `el.text.fill` (emphasis), 飾り方 `text.style`, 向き `orient`, 位置・回転・拡大 `el.text.nudge` (also by dragging handles on the preview), 非表示 `el.text.hide`, 文字の出方/間隔 (`arrive.order`, `arrive.each`), 「この部品の調整」 for the chosen arrange/arrive/dwell/depart part params |
...
1. **Connection card**: サービス (Google Gemini default / Anthropic Claude) · モデル · APIキー (masked, show toggle) · key
...
- **≡ menu**: ファイル (新しく作る · 開く… Ctrl+O · 保存 Ctrl+S · 名前を付けて保存… Ctrl+Shift+S · 最近の作品 ▸ (5 autosaves) ·
  ...
  自動再生 · AIを使う) · ヘルプ (キーボード操作 ? · 記法のヘルプ · このアプリについて・ライセンス) · 言語 English / 日本語.
...
without a song). Space/Enter = start of the next line (and advance); E = end of the current line; Backspace = back one line
...
| 曲を使う without a song / with Claude | disabled with the reason |
...
### 6.14 UI files (`next/src/ui/`, WP8; "pure" = no DOM at factory time and Node-tested)
...
| `project_io.js` | new/open/save/recent/.lrc, autosave to IndexedDB (doc + side; song blob by sha1), global drop routing |
...
with `cost ≥ 3` at half resolution, (2) `draft` paints, (3) DPR 0.75, (4) skip filters with `cost ≥ 4` (badge 軽量表示) —
...
Node tests live in `next/tests/node/*.test.js` and run with `node --test next/tests/node` (Node 22, no npm packages).
Browser tests live in `next/tests/browser/*.py` and use the launcher `dev/browser.py`. CI runs, in order:
`python3 next/build.py --check`, the Node tests, `python3 next/build.py --lab`, `python3 next/build.py`, the browser tests,
and the originality checks (§8.4).
...
| `planner_stability.test.js` | WP3 | insert a line: ≥ 98% of other cuts' choices unchanged on average, ≤ 4 changed per case; add a part: ≥ 90% unchanged; reroll a cut: ≤ 3 other cuts change |
...
| `audio.test.js` | WP6 | synthetic click tracks at 90/128/174 BPM: bpm within ±1, offset within 20 ms; envelope and `digest` round trip; `level` interpolation; `encodeWav` bytes equal the 11t implementation's output for fixed inputs |
...
| `ai_providers.test.js` | WP7 | ported from `dev/ai_test.js`: Gemini request shape, key only in the header, thought parts ignored, error mapping, no key → nothing sent, Claude SDK modes (opus fallback beta, haiku without thinking), refusal/max_tokens mapping, key check, key shape hint, schema portability |
| `ai_lyrics.test.js` | WP7 | ported: prep validation (cuts, emphasis, readings), `renderChecked` refusals, removals keep per-line settings (now by id), blank squeezing, stale rows skipped |
| `ai_looks.test.js` | WP7 | ported: proposals (3, season set, off-season/unknown dropped), palette accent fitted, edit changes only real changes, understood=false → none, lyric words never change; new: every change maps to the commands of §4.22.5 and applies as one batch |
| `ai_song.test.js` | WP7 | ported from `dev/ai_audio_test.js`: WAV header/size, rate picking, base64, inline vs Files API upload, upload errors, audio part before the prompt, seconds(), transcript cleaning and LRC parse-back, `#` lines, alignment rules (now pins by `ai`, allowed over LRC with the review note), song info cleaning and context |
...
### 8.3 Browser tests (`next/tests/browser/*.py`)
...
### 8.4 Originality checks (CI; only scripts read the forbidden files)

1. `python3 dev/originality_check.py --min-run 12` — token shingles shared between `next/` and the JIZURA file list
   (exits 1 on any hit). The window is 12 tokens (stricter than the script's default).
2. `python3 next/tests/originality_names.py` — extracts identifiers and string-literal keys from the forbidden files and
   compares them with v2's registry keys (parts, themes, moods), exported function names and string-table keys. It prints only
   the overlapping names (never code). Overlaps fail CI unless the name is in `next/tests/originality_allow.txt` (generic
   English words and web API names only), reviewed by the lead.
3. Every PR carries the clean-room attestation line (§1.4); the PR template checks for it.
4. File header rule and the ban on reasoning about JIZURA apply to reviews: a review comment that describes JIZURA internals
...
- **Files**: `next/build.py`; `next/THIRD_PARTY_NOTICES.md`; `src/core/define.js`, `types.js` (JSDoc `@typedef`s that mirror
  ...
  `tests/originality_names.py`, `tests/originality_allow.txt`, `tests/update_golden.js` (rewrites `tests/golden/*` on purpose);
...
- **Acceptance**: every part passes `conformance.test.js` and `parts_gallery.py`; `createRegistry(defaultRegistry defs)`
  validates in strict mode; counts ≥ §5; the planner variety/stability tests pass with the full catalog; the originality
  names check passes. Each PR adds parts of one file/family and includes a lab screenshot per part.
...
#### WP7 — AI port (1 engineer, ≈ 2,200 LOC)

- **Files**: `src/ai/providers.js`, `lyricio.js`, `catalog.js`, `prep.js`, `looks.js`, `song.js`, `changes.js`; tests
  `ai_providers`, `ai_lyrics`, `ai_looks`, `ai_song` (cases ported from `dev/ai_test.js` and `dev/ai_audio_test.js`).
...
- **Acceptance**: ported test cases all pass on the v2 model; every Change kind converts to commands and applies as one batch;
...
MV.use('ai/providers').setSDK(window.AnthropicSDK);
...
| M3 | weeks 6–8 | full catalog; AI port complete; UI complete incl. AI panel, palette, timeline drawer |
| M4 | weeks 9–10 | performance, browser matrix, en UI, originality gates, release candidate |
...
1. **AI alignment over LRC stamps.** Default: allowed; results are pins by `ai`, which win over stamps, and the review row
   says 「LRCの時刻より優先されます」. (The previous app refused alignment when stamps existed.)
...
5. **Phase 2 options** (not in v2.0): export rendering in a Worker (then CSP `worker-src blob:`); an own fragmented-MP4
   ...
   page. The spec currently keeps both vendor files and their MIT notices.
6. **License statement.** v2 contains no JIZURA code, data or catalog by construction and by CI check; whether the JIZURA
   MIT notice can then be dropped from v2's distribution is the owner's decision (legal, not technical). The vendor MIT
