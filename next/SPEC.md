...
v2 replaces the JIZURA-based app with an **original** engine and UI. It must not contain, translate or paraphrase JIZURA
code, data tables or part catalogs. This file describes behaviour only, from the outside.

## 0. Clean-room rules (for everyone who writes v2 code)

- **Do not open** these JIZURA files while working on v2: `src/0*.js`, `src/10_audio.js`, `src/11_export.js`,
  `src/11p_*.js`, `src/11q_sets.js`, `src/12_ui.js`, `app/`, `build.py`, `docs/EXPRESSION_PACKS.md`, `index.html`,
  `en/index.html`, `dev/*.js`
  except our own tests named below, and `dev/snapshots/`.
- Allowed references: this spec, `next/DESIGN.md`, general web/graphics knowledge, and our own code:
  `src/11s_ai.js`, `src/11t_ai_audio.js` (AI behaviour to port), `dev/ai_test.js`, `dev/ai_audio_test.js`,
  `dev/browser.py`. Third-party: `vendor/mp4-muxer.min.js` (MIT, keep its notice), `vendor/ai-sdk.min.js` (MIT).
- Names of parts, themes and moods are v2's own. Do not reuse JIZURA's lists.
- v2 lives in `next/`. It builds to one self-contained page `next/index.html` (and `next/en/index.html`).
...
## 8. AI assistant (optional, same behaviour as today's app)

Port the behaviour of `src/11s_ai.js` / `src/11t_ai_audio.js` to the v2 data model:
...
- Originality check: no long code passages shared with JIZURA (`dev/originality_check.py`).

## 10. Not in v2.0

After Effects export / panels. (Old app stays reachable until v2 replaces it.)
