/* 文字PVメーカー v2 — original work. Tap mode: Space/Enter marks line starts while the song plays; one time.tap on finish (DESIGN §4.11, §6.4.15). */
MV.def('ui/tap', ['ui/dom', 'ui/icons', 'ui/keys', 'ui/selection', 'i18n/t'], (dom, I, K, S, T) => {
  ...
  function mount(app) {
    ...
    function mark(args) {
      if (!session) return;
      if (reduce('mark', eventTime(args))) announce('tap.marked', lineAt(session.state.current));
      update();
    }

    function end(args) {
      if (!session) return;
      if (reduce('end', eventTime(args))) announce('tap.endMarked', lineAt(session.state.current));
      update();
    }
    ...
    function finish() {
      ...
      if (cmd && n) {
        ...
        app.toast(t('tap.done', { n }), { kind: 'ok' });
      }
    ...
    function announce(key, line) {
      if (line) last.textContent = t(key, { n: line.index + 1, time: T.fmtTime(app.time()) });
    }
