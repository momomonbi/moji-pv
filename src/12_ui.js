...
const LS_KEY = 'mojipv.project.v1';
...
/* 季節（AI 演出3案・ひとこと修正で設定。11r_motifs.js） */
const SEASON_NAME = { spring: '春の歌', summer: '夏の歌', autumn: '秋の歌', winter: '冬の歌', none: '季節なし' };
J.SEASON_NAME = SEASON_NAME;
/* why random picks skip this part / style (追加分 → 和風 → 季節, the order J.randomOk checks them) */
function offReason(g, d, k, auto) {
  const when = auto ? '自動では選ばれません）' : 'おまかせでは選ばれません）';
  if (d.extra && S.project.extra !== true) return '（追加分がオフのため、' + when;
  if (d.wa && S.project.wa === false) return '（和風の演出がオフのため、' + when;
  return '（季節に合わないため、' + when;
}
...
function loadThumbFonts() {
  ...
  thumbFonts = J.ensureFonts('文字', [...new Set(J.STYLE_ORDER.map(k => J.STYLES[k].fonts.display[0]))]).then(() => drawStyleGrid()).catch(() => {});
}
...
function drawStyleGrid() {
  ...
  [...g.children].forEach(b => {
    ...
    b.title = st.desc + (off ? offReason('style', st, k, false) : '');
    ...
    x.fillStyle = sc.ghostB; x.fillText('文字', 96 - 3, 54 - 1);
    x.fillStyle = sc.ghostA; x.fillText('文字', 96 + 3, 54 + 2);
    x.fillStyle = sc.fg; x.fillText('文字', 96, 54);
...
const HKEYS = ['style', 'mood', 'seed', 'fx', 'enabled', 'fonts', 'colors', 'overrides', 'season'];
const H = { list: [], i: -1 };
...
function showNow() {
  ...
  el.innerHTML = row('スタイル', `<b>${escapeHtml(J.STYLES[P.style].name)}</b>`)
    ...
    + (P.season && SEASON_NAME[P.season] ? row('季節', escapeHtml(SEASON_NAME[P.season])) : '')
...
function setMode(m) {
  ...
  try { localStorage.setItem('mojipv.mode', S.mode); } catch (e) {}
...
function renderTech() {
  ...
  GROUPS.forEach(([g, label]) => {
    ...
    shown.forEach(k => {
      ...
      if (!J.randomOk(S.project, g, k)) { l.classList.add('set-off'); l.title += offReason(g, tbl[k], k, true); }
...
function baseName() {
  ...
  return ((S.project.title || 'moji-pv').replace(/[\\/:*?"<>|]+/g, '_').slice(0, 60) || 'moji-pv') + (k ? (k === 'green' ? '_greenback' : '_blackback') : '');
}
...
function bind() {
  ...
  $('btnSave').addEventListener('click', () => J.saveFile(baseName() + '.mojipv.json', JSON.stringify(S.project, null, 1)));
...
function boot() {
  S.project = loadLocal();
  bind(); syncUI(); replan();
  let mode = 'easy'; try { mode = localStorage.getItem('mojipv.mode') || 'easy'; } catch (e) {}
  setMode(mode); commit();
...
J.uiApi = { toast, replan, syncUI, pause, seek, flushSave, loadAudioFile, restartPreview,
  // for the AI panel (13_ai_ui.js): history stops around a change, and a full refresh after the project is replaced
  remember, commit, refresh() { fontKey = ''; syncUI(); replan(); flushSave(); }, isBusy: () => !!(S.exporting || S.tap),
  // renumber per-line settings in the ◀ ▶ history after lines were removed (map: old line number -> new; missing = gone)
  remapLines(map) {
    H.list = H.list.map(snap => {
      const o = JSON.parse(snap);
      if (o.overrides) { const n = {}; for (const [k, v] of Object.entries(o.overrides)) if (map[k] != null) n[map[k]] = v; o.overrides = n; }
