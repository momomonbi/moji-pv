...
/* technique subset per group for a mood: everything tagged with the mood (plus the mood's hand-picked core items),
   a sprinkle of everything else, and a minimum count so the planner always has room to vary.
   (used by おまかせ and by the AI proposals; consumes rnd in a fixed order) */
J.moodEnabled = (project, mood, rnd = Math.random) => {
...
J.omakase = (project, rnd = Math.random) => {
  ...
  const enabled = J.moodEnabled(project, mood, rnd);
