/* 文字PVメーカー v2 — original work. explain(): why a slot has its value, and the alternatives — lazy, never in the Plan (DESIGN §4.16.8). */
MV.def('planner/explain', ['core/paths', 'core/pins', 'core/lyrics', 'planner/choose', 'planner/look', 'planner/plan',
  'planner/fields'], (P, PINS, LY, CH, LK, PL, F) => {
  'use strict';
  ...
  function factorWhy(registry, plan, cut, trace, kind, value, seasonWord) {
    const why = [];
    ...
    if (cand && cand.f && def) {
      ...
      if (f.season !== 1) why.push({ code: 'season', params: { season: plan.look.season.v, word: seasonWord || '' } });
  ...
  // Why a registry part is not a candidate here: 'filter' | 'season' | 'gate' | 'role' | 'trait' | null.
  function maskOf(registry, doc, plan, kind, key, ctx) {
    const def = registry.get(kind, key);
    const t = registry.traits(kind, key);
    const season = plan.look.season.v;
    if (filterMasks(doc.filters, kind, key)) return 'filter';
  ...
  function explainRead(doc, plan, parsed, registry, cut) {
    ...
    const value = F.valueAt(plan, cut, parsed, registry);
    const ix = PINS.index(doc.pins);
  ...
  // scope for backgrounds, atmospheres and seams (they belong to a segment or a boundary, not to the cut's text).
  function maskContext(plan, cut, parsed, kind, trace) {
    const slotKind = parsed.part.kind;
    if (kind === 'ground') return {};
    if (kind === 'seam') return { scope: trace.world ? 'world' : 'text' };
    if (slotKind === 'atmos') return { scope: 'run' };
    return {
  ...
  function tracedWhy(doc, registry, plan, cut, parsed, trace, d, value) {
    if (d.from && d.from.startsWith('pin')) return pinWhy(d.from, d.by);
    ...
    if (F.categoryOf(parsed) === 'part' && trace.candidates) {
      ...
      return factorWhy(registry, plan, cut, trace, trace.kind || parsed.part.kind, value, seasonWord);
    }
