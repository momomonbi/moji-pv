/* 文字PVメーカー v2 — original work. Seams: the transition part of a plan seam, its params and the guarded mix of two surfaces (DESIGN §4.19.2 step 4). */
MV.def('engine/render/seam', ['core/hash', 'engine/scene/table', 'engine/render/post'], (H, T, PO) => {
  'use strict';
  ...
  const resolved = new WeakMap();        // plan seam entry → { def, p, seed }

  // seamPart(plan, registry, i) → { def, p, seed } for plan.seams[i] (p.dur = the window's length); an unknown key falls
  // back to the kind's fallback.
  function seamPart(plan, registry, i) {
    ...
    hit = { def, p, seed, registry, unknown };
    resolved.set(s, hit);
    return hit;
  }

  // mix(ctl, pool, part, a, b, u) → Surface: part.def.mix(fx, a, b, u, p), clamped u, checked result. A part that throws
  // or returns something that is not a surface gives a hard cut (a before u = 0.5, b after) and the error, so a broken
  // transition never breaks the frame.
  function mix(ctl, pool, part, a, b, u, onError) {
    const k = u < 0 ? 0 : u > 1 ? 1 : u;
    ctl.use(part.seed, null, false);
    try {
      const out = part.def.mix(ctl.fx, a, b, k, part.p);
